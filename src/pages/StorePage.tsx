...
const StorePage = () => {
  ...
  useEffect(() => { 
    if (vendorId) fetchStoreDetails(); 
  }, [vendorId]);

  const fetchStoreDetails = async () => {
    try {
      const id = typeof vendorId === 'object' ? (vendorId as any).id : vendorId;
      ...
      if (storeRes.ok) setStore(storeData.data || storeData);
      if (prodRes.ok) setProducts(Array.isArray(prodData) ? prodData : (prodData.data || []));
    } catch (error) {
  ...
  const handleProductClick = (product: any) => {
    ...
    setSelectedOptions({}); // අලුත් නිෂ්පාදනයක් තෝරාගත් විට options reset වේ
  };

  const handleAddToCart = () => {
    const requiredVariants = Object.keys(selectedProduct.variants || {}).length;
    const selectedCount = Object.keys(selectedOptions).length;
    if (requiredVariants > 0 && selectedCount < requiredVariants) {
      alert("Please select all product options!");
      return;
    }
    console.log("Added to cart:", { ...selectedProduct, selectedOptions });
  };
    ...
    <div className="min-h-screen bg-[#c5a67e] pb-20">
      <div className="bg-[#2D2A26] text-white p-10 rounded-b-[3rem] shadow-2xl">
        ...
        <div className="flex flex-wrap gap-4 mt-4 text-sm font-medium opacity-80">
          ...
          <span className="flex items-center gap-1">📞 {store.phone || "No Phone Added"}</span>
          ...
          <div key={category} className="mb-12">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {groupedProducts[category].map((product: any) => (
                <motion.div key={product._id} whileHover={{ y: -5 }} className="bg-white p-4 rounded-3xl shadow-sm border border-gray-100 cursor-pointer" onClick={() => handleProductClick(product)}>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4" onClick={() => setSelectedProduct(null)}>
            <motion.div initial={{ scale: 0.9 }} animate={{ scale: 1 }} exit={{ scale: 0.9 }} className="bg-white p-8 rounded-[2rem] max-w-lg w-full relative shadow-2xl max-h-[90vh] overflow-y-auto" onClick={(e) => e.stopPropagation()}>
              ...
              <div className="flex items-center justify-between mt-6 bg-gray-50 p-4 rounded-2xl">
                 ...
                 <span className="text-xs font-bold text-gray-500">{selectedProduct.deliveryCharge === 0 ? "✨ Free Delivery" : `Delivery: LKR ${selectedProduct.deliveryCharge?.toLocaleString()}`}</span>
              </div>
                  ...
                  <div key={key}>
                    ...
                    <div className="flex flex-wrap gap-2 mt-2">
                      {val.split(",").map((opt: string) => {
                        ...
                        return (
                          <button key={optionValue} onClick={() => setSelectedOptions((prev: any) => ({...prev, [key]: optionValue}))} className={`px-4 py-2 rounded-xl border-2 font-bold transition ${selectedOptions[key] === optionValue ? "bg-[#8B5E3C] text-white border-[#8B5E3C]" : "border-gray-200"}`}>{optionValue}</button>
                        );
                      })}
                    </div>
                  </div>
              ...
              <button onClick={handleAddToCart} className="mt-8 w-full py-4 bg-[#2D2A26] text-white rounded-2xl font-black uppercase tracking-widest hover:bg-black transition">Add to Cart</button>
            </motion.div>
          </div>
        )}
      </AnimatePresence>
