...
const MainLayout = ({ children }: MainLayoutProps) => {
  ...
  return (
    <div className="flex min-h-screen bg-[#FAF9F6] relative">
      ...
      {/* Aside - Sidebar */}
      <aside className={`fixed lg:static w-72 h-full ${asideBg} p-6 flex flex-col justify-between shadow-2xl transition-transform duration-500 ease-in-out z-40 ${isOpen ? "translate-x-0" : "-translate-x-full lg:translate-x-0"}`}>
  <div className="flex flex-col h-full overflow-hidden"> {/* h-full සහ overflow-hidden එකතු කළා */}
    
    {/* Brand Name */}
    <div className="hidden lg:flex justify-center mb-4"> {/* mb-10 -> mb-4 */}
      <h1 className="text-2xl font-black uppercase italic tracking-widest text-[#D4A373]">VelvetMart</h1>
    </div>

    {/* User Profile */}
    <div className="mt-2 lg:mt-0 mb-6 border-b border-white/10 pb-4 flex flex-col items-center"> {/* mb-10 -> mb-6, pb-8 -> pb-4 */}
      <div className="w-16 h-16 rounded-full bg-[#5C5048] flex items-center justify-center text-white text-2xl font-black mb-2 shadow-xl border-2 border-[#D4A373]">
    ...
    {/* Navigation Links - space-y-3 -> space-y-1 */}
    <nav className="flex-1 overflow-y-auto space-y-1"> 
      {navLinks.map((link) => (
        <Link 
          ...
          className={`block text-[12px] font-bold uppercase pl-4 py-2 transition-all duration-300 rounded-r-lg ${
  ...
  <button 
    onClick={logout} 
    className={`w-full py-2.5 px-4 border ${isVendor ? "border-[#D4A373]/50 text-[#D4A373]" : "border-[#8B5E3C]/50 text-[#8B5E3C]"} rounded-xl font-black uppercase hover:bg-[#D4A373] hover:text-white transition-all duration-300 shadow-md mt-4`}
  >
    Logout
  </button>
</aside>
